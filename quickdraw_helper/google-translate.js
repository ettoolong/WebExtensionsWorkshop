// source code from another addon:
// gtranslate 0.13.1, https://addons.mozilla.org/zh-TW/firefox/addon/gtranslate/
...
function translate(from, to, text, cb) {
  ...
  const onComplete = event => {
    const translation = translationResult(event.target.responseText, () => {
  ...
  if (text.length < 200 ) {
    ...
    request.open("GET", apiUrl(from, to, text, true), true);
    request.send();
  } else {
    //console.log('text.length >= 200');
    let request = new XMLHttpRequest();
    request.onload = onComplete;
    request.onerror = onError;
    request.open("POST", apiUrl(from, to, text, false), true);
    request.setRequestHeader("Content-Length", 'q='.concat(encodeURIComponent(text)).length);
    request.send('q='.concat(encodeURIComponent(text)));
  }
}
